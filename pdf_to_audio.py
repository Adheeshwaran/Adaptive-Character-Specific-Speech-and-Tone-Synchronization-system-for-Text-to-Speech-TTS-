...
pdf_path = 'D:/69th_stop/pdf_audio_llm/the-picture-of-dorian-gray-updated.pdf'  # Replace with the actual PDF file path
...
def text_to_speech_female_2(input_text, output_file, model="tts-1-hd", voice="shimmer"):
    # Initialize the OpenAI client
    client = openai.OpenAI(api_key = "-abc-")
    ...
    for i1, chunk in enumerate(chunks):
        ...
        for i in person_names:
            ...
            if g_list[-1]=="_":
                ...
                # Define the path for the output audio file
                output_file = os.path.join(output_folder, f"chunk_{i1+1}.mp3")
...
combine_audio_with_moviepy('chunks1', 'combined_audio.mp3')  # Combine audio files in 'chunks' folder
